#!/usr/bin/env python3
"""核心处理引擎：文件解析 / 现金流规则分类 / Grok 4.5 分类与问答 / 导出。
...
def _ai_chat(messages: List[Dict[str, str]], *, temperature: float = 0.1,
    ...
    url = config.AI_BASE.rstrip("/") + "/chat/completions"
    headers = {"Authorization": f"Bearer {config.AI_KEY}", "Content-Type": "application/json"}
    body = {
...
def grok_batch(items: List[Dict[str, Any]], user_rules: str,
    ...
    if not config.AI_KEY:
        return [{"id": it["id"], "category": "待确认", "reason": "未配置 AI Key",
                 "confidence": 0.0, "review": True, "source": config.AI_MODEL_NAME} for it in items]
    user = {
    ...
    try:
        ...
        return [{"id": it["id"], "category": "待确认", "reason": f"{config.AI_MODEL_NAME} 调用失败：{e}",
                 "confidence": 0.0, "review": True, "source": config.AI_MODEL_NAME} for it in items]
    ...
    for it in items:
        ...
        out.append({
            "id": it["id"],
            "category": cat,
            "reason": res.get("reason") or f"{config.AI_MODEL_NAME} 未返回该行结果",
            "confidence": conf,
            "review": bool(res.get("review")) or conf < 0.8 or cat == "待确认",
            "source": config.AI_MODEL_NAME,
        })
    return out
...
def grok_answer(question: str, context: Dict[str, Any], accounting_skill: str | None = None) -> str:
    ...
    try:
        ...
        return f"{config.AI_MODEL_NAME} 调用失败：{e}"
...
def classify_job(job: Dict[str, Any], learned_rules: List[Dict[str, Any]] | None = None, custom_skill: str = "") -> Dict[str, Any]:
    ...
    for start in range(0, len(ai_items), BATCH):
        ...
        try:
            ...
            ai_res = [{"id": it["id"], "category": "待确认", "reason": f"{config.AI_MODEL_NAME} 调用失败：{e}", "confidence": 0.0, "review": True, "source": config.AI_MODEL_NAME} for it in chunk]
        by_id = {x["id"]: x for x in ai_res}
        for it in chunk:
            i = it["id"]
            res = by_id.get(i, {"id": i, "category": "待确认", "reason": "无结果", "confidence": 0.0, "review": True, "source": config.AI_MODEL_NAME})
...
def build_report_text(job: Dict[str, Any]) -> str:
    ...
    if chats:
        lines.append(f"【{config.AI_MODEL_NAME} 处理/问答记录】")
        ...
        lines.append(f"还没有生成 {config.AI_MODEL_NAME} 处理结果。")
    return "\n".join(lines)
